#Release 0:
# Open your release_1.rb file and read the code 
# (especially the comments). Run the file. 
# Is there anything that you see that you do not 
#   understand? Look up or ask questions about 
#   anything unfamiliar.

# Run the code. Do you notice what changes?
# Create the class
class Profile

# Initializes every time you create a new instance of Profile class
# Sets the initial instance variable values which can be called throughout the class, not just within a method
  def initialize
    @age = 27
    @name = "Kim"
...
# just prints info - the puts on lines by themsleves are new lines
  def print_info
    puts
    puts "age: #{@age}"
    puts
...
# returns a value outside of the class
  def what_is_age
    @age
  end
...
# changes the values age, name, and occupation and resets them equal to 
